...
import {
  ...
  isMappedRegionCategory, setCollectionSourceUrl, setCollectionCover, findLatestAssetBySourceUrl,
  resetAssetForOverwrite, deleteGroupAssetsExcept, findCollectionBySourceUrl,
...
export function detectMixId(url: string): string | null {
  // /mix/ 与 /collection/ 是老式合集；/share/playlet/detail/ 是抖音「长视频合集/剧集」
  // 分享链常落的格式（如妆教长视频合集），object_id 即合集 id，f2 的 fetch_user_mix_videos 同样能枚举。
  const m = url.match(/\/mix\/detail\/(\d+)/)
    || url.match(/\/collection\/(\d+)/)
    || url.match(/\/playlet\/detail\/(\d+)/);
  if (m) return m[1];
  try {
    const u = new URL(url);
    if (/mix|playlet/i.test(u.pathname)) {
...
const ANALYZE_SYS = `你是"回响"的内容结构引擎。把一条视频的转写文本重构成一条可导航的"瘦脉络"。只输出 JSON 对象，不要多余文字。

第一步，不按题材或体裁分类，只判定哪套渲染结构能忠实承载视频中一个完整、重要、可独立理解的核心区块（五选一）。特殊结构不需要占据整条视频；只要它能形成有信息量的完整视图，就应优先于通用线性脉络。

下面五套结构按「越特殊越靠前」排列（data > scenario > compare > history > argument）。**请从上往下逐一检验，命中第一套能从原文凑齐完整字段的结构就停下**，不要继续下滑到更宽松的类型。argument 和 history 是最后两道兜底，只有当上面三套特殊结构都无法成立时才在它们之间二选一——绝不能因为内容「也能讲成一个主张」或「也能按顺序讲」就跳过前面的检验。

- data 量化关系：原文给出至少 3 个可直接对照的数值，它们属于同一口径或同一竞争集合，能形成大小、占比、排名或差异的完整结论。数值只需构成一个重要核心区块，不要求整条视频都在报数据。
- scenario 条件分支：视频里存在一个可切换的关键条件（用户状态、场景取值、某个选择或假设），它一旦改变就会改写后续的行动或结论链——本质是「结论取决于这个条件」。只要能拎出这个 pivot 条件、及其两种取值各自导向的不同结果，就成立；两条分支不必等长、不必对称，视频不必自称「分支」，也不必整条都在讲它。凡是「分情况 / 取决于 / 如果是 A 就…如果是 B 就…」这类结构，优先判 scenario，不要摊平成 argument，也不要因为它顺带有先后就归 history。可以是决策、诊断、方案选择或反事实。
- compare 矩阵对比：有至少 2 个可比对象，并在至少 2 个共同维度上逐一对照。注意「提到多个对象各自的优缺点」不等于 compare，必须是同一组维度逐格套到每个对象上、能填成一张对照表才算。
- history 顺序推进：核心信息必须按时间、步骤或阶段展开；可以是历史演进、事件经过、教程、跟练或操作流程，不要求必须有年份。但「有先后顺序」本身不构成选 history 的理由——若这些步骤/对象之间其实是多对象逐维对照、两条并行的条件分支、或一组同口径可比数值，应分别优先判 compare、scenario、data；只有当内容凑不齐那三套结构、只剩一条线性推进时才归 history。
- argument 线性脉络：观点、原理、因果或普通说明构成一条主线，并且上面四套特殊渲染器都无法从原文中得到完整数据。argument 是最后兜底，不是拿不准时的安全项。

正向校准：
- “训练市场 A 占 90% 以上、B 占 3%~5%、C 不足 1%”已能构成完整份额图，应判 data；即使视频其它部分还在解释竞争原因。
- “你是干皮还是油皮 → 分别采取 A/B 护理行动”是用户状态决定行动路径，应判 scenario。
- “左侧与右侧策略在不确定性、确认时点、风险代价上有何差异”是两个对象的多维度对照，应判 compare，不是 scenario。
- 只是散落提到无关数字，或只有“如果”句式却没有两套完整行动/结果，不足以选 data/scenario。

同时成立时优先级：data > scenario > compare > history > argument。这不是备注，而是强制判定顺序：先依次检验 data、scenario、compare 能否各自凑齐完整字段，任一成立就选它，不得因为内容「也能按顺序讲」就跳过它们去选 history。只有当 data、scenario、compare 三套都无法成立时，才在 history 与 argument 之间二选一；选 history 要确认存在真实的时间/步骤/阶段线，选 argument 前必须逐一说明为什么原文无法生成 data、scenario、compare、history 的完整字段。

第二步，严格按所选结构组织脉络：
- argument: 核心问题/主张 → 原因或拆解 → 依据/案例 → 结论或边界
- history: 起点 → 步骤/阶段/节点×2~5 → 结果；保留原文的时间、步骤号或顺序标记
- compare: 共同问题 → 对象 → 共同维度 → 差异 → 选择/结论
- data: 量化问题 → 数据口径 → 关键数值 → 差异 → 数值意义
- scenario: 固定前提 → 可变条件 → 分支A → 分支B → 选择边界

输出字段：
- core_question: 这条视频在回答的核心问题（一句话）
- video_type: "argument" | "history" | "compare" | "data" | "scenario"
...
- backbone: 4~7 个概念节点（少于4或多于7都不行），数组顺序=一条脉络线；每个 {id, concept, role, detail, timestamp}
    ...
    · role: 该节点在所选骨架中的环节名，≤4字（如"主张/依据/结论"、"起点/步骤/结果"、"对象/维度/差异"、"口径/数值/意义"、"前提/分支/边界"）
...
export const VIDEO_TYPES: ReadonlySet<string> = new Set(ANALYSIS_TEMPLATES);
...
/* ---------- 模板实例化：video_type 已与五套渲染结构一一对应。
   这里不再二次分类，只把已选结构整理成对应 renderData。 ---------- */
const TEMPLATE_SYS = `你是"知音"的渲染数据编排引擎。输入已经确定的 structure_type、脉络与转写原文。你不得重新分类；template 必须与 structure_type 完全相同，只生成该渲染器需要的结构化数据。只输出 JSON 对象，不要多余文字。

结构与渲染器是固定对应：
- history：顺序推进→时间轴。既可以是年代演进，也可以是教程步骤、操作流程或阶段变化。
- compare：矩阵对比→双行/多行对照表。
- data：量化关系→横向柱状图。
- scenario：二值条件分支→条件开关与两条结果链。可以是决策、诊断、方案选择或反事实，不限于历史。

必须忠实于转写与 backbone，不得为了补字段编造数字、对比维度、步骤或分支结果。
...
"history": { "events": [{"year":"年份/时间/第1步/阶段名","short":"≤6字顺序节点简称","role":"≤4字环节名（如起点/步骤/结果）","title":"≤16字","detail":"2~4句","sourceNodeIndex": 对应 backbone 下标，没有就不给}], "defaultIndex": 默认展开第几项（0起，可省略） }
...
"scenario": { "condition":"可在两种取值间切换的条件问句", "offState":"条件A的简短文案", "onState":"条件B的简短文案", "premise": {"title":"≤16字","detail":"2~4句","sourceNodeIndex": 可省略}, "off": [{"tag":"≤6字的A分支标签","title":"≤16字","detail":"2~4句"}], "on": [{"deform":"shift|broken|fork","tag":"≤6字的B分支标签","title":"≤16字","detail":"2~4句"}] }

铁律：template 必须原样输出 structure_type，不得改成 argument 或其它类型。行文用中文标点，引用与强调用「」，禁止出现"视频1""视频2"这类指称。字符串内部的引号、换行必须转义，务必输出合法 JSON。`;
...
export async function classifyTemplate(
  ...
  const fallback = (reason: string) => createArgumentDispatch({
  ...
  // 线性脉络本身就是 argument 渲染器需要的完整数据，无需再调一次模型。
  if (target === "argument") return fallback("structure-aligned:argument");
  ...
  const input = JSON.stringify({
    ...
    // 必须与 analyzeTranscript 的判型视野一致：否则第一阶段可能用
    // 6k~12k 内的数值/分支选中模板，渲染阶段却看不到同一批证据。
    transcript_excerpt: transcript.slice(0, 12000),
  });
  ...
  const attempt = async (): Promise<AnalysisDispatch> => {
    ...
    if (parsed.template !== target)
      throw new Error(`渲染结构错位（要求 ${target}，返回 ${parsed.template ?? "empty"}）`);
    const template = target;
    const confidence = clampConfidence(a.type_confidence);
    const reason = String(parsed.reason ?? "").trim() || `structure-aligned:${target}`;
    ...
    if (template === "history") {
      ...
      if (premise && off.length >= 1 && on.length >= 1) {
        renderData = {
          ...
          offState: String(parsed.scenario?.offState ?? "").trim() || "条件 A",
          onState: String(parsed.scenario?.onState ?? "").trim() || "条件 B",
          premise,
          off,
          on,
        };
    ...
    if (!renderData) throw new Error(`${target} 渲染字段不完整`);
  ...
  for (let i = 0; i < 2; i++) {
    try { return await attempt(); }
    catch (error) { lastError = error; /* 换一次采样重试 */ }
  }
  const detail = lastError instanceof Error ? lastError.message.slice(0, 80) : "unknown";
  return fallback(`template-render-failed:${target}:${detail}`);
}
...
// 产品策展例外：这组日常内容不形成独立主题岛，固定留在「日常 · 散篇集」。
// 从模型输入中排除，保证自动收尾和手动重聚类都不会再次把它们搬走。
const FORCED_MISC_ASSET_IDS = new Set([
  "f4153ce6",
  "04ab314f",
  "07d34553",
  "bdd3997d",
...
export async function reclusterMisc(cat: CategoryId, dryRun = false): Promise<ReclusterResult> {
  const allMiscAssets = listAssetsByCollection(MISC_COLLECTION[cat].id);
  const miscAssets = allMiscAssets.filter((asset) => !FORCED_MISC_ASSET_IDS.has(asset.id));
  const existing = listCollections(cat).filter((c) => c.id.startsWith("tc-"));
  const result: ReclusterResult = {
    category: cat, remainingMisc: allMiscAssets.length, assigned: [], created: [],
  };
  ...
  result.remainingMisc = allMiscAssets.length - moved;
...
二、episode_labels：必须覆盖输入里的每一条视频，每项 {video_id, label}：
- video_id：必须原样使用输入 videos 里的真实 id，每条恰好一次。
- label：先综合该集的 title、core_question 和全部 nodes，再提炼成 **2~8 个可独立理解的中文短字**（柱下分两行显示，能容两行，别硬压成一个词）。它要回答“这一集主要在讲什么”，优先使用人物、事件、机制或争点，如「杠杆收购」「雷曼倒下」「加息代价」「三星半导体崛起」。
- 禁止截取标题前几个字，禁止「第一集」「本集主题」等无信息标签，禁止只写宽泛大类词。同一合集内各标签必须能彼此区分。

三、points：2~4 个跨视频知识点，每个是"多条视频在同一件事上的关系"，不是单条视频的摘要。每条 {label, relation, stance?, note, sources}：
...
行文用中文标点，引用与强调用「」。只输出 JSON：{"series_question":"…","episode_labels":[{"video_id":"…","label":"…"}],"points":[{"label":"…","relation":"…","stance":[{"tag":"a","text":"…"}],"facets":[{"lead":"…","label":"…","detail":"…","focus":"…","source_ids":["…","…"]}],"sources":[{"video_id":"…","timestamp":"…"}]}]}
...
export async function generateSynthesis(videos: SynthesisInputVideo[]): Promise<Synthesis> {
  ...
  const attempt = async (): Promise<Synthesis> => {
    const parsed = await callDeepseekJson<{
      ...
      episode_labels?: { video_id?: string; label?: string }[];
      points?: {
    ...
    // 短签在柱下分两行显示（reader.css .scEpisodeLabels 两行 clamp），
    // 超长不再整条丢弃、连累整个合集合成失败，而是截到两行容量（8 字）。
    const episodeLabels = (Array.isArray(parsed.episode_labels) ? parsed.episode_labels : [])
      .map((item) => ({
        videoId: String(item?.video_id ?? "").trim(),
        label: Array.from(String(item?.label ?? "").replace(/[\s，,。！？?!：:；;、｜|]/g, "").trim()).slice(0, 8).join(""),
      }))
      .filter((item, index, all) =>
        byId.has(item.videoId) &&
        Array.from(item.label).length >= 2 &&
        all.findIndex((candidate) => candidate.videoId === item.videoId) === index
      );
    if (episodeLabels.length !== videos.length) throw new Error("每集短签不完整");
    ...
    return { seriesQuestion, episodeLabels, points };
  };
...
export interface MixEnumeration {
  videos: MixVideo[];
  mixName: string;
  coverUrl: string;
}
...
export async function enumerateMix(mixId: string): Promise<MixEnumeration> {
  const data = await runMixScript<{
    ok: boolean;
    error?: string;
    videos?: MixVideo[];
    mix_name?: string;
...
/** 去掉分享参数带来的表面差异，让同一条抖音视频稳定命中同一个来源。 */
function normalizeSourceUrl(url: string): string {
  const awemeId = detectAwemeId(url);
...
export async function runAssetPipeline(assetId: string): Promise<void> {
  ...
  try {
    ...
    // L5：回响先行——五模板要把 echo 织进 renderData（history 逐事件 / data·scenario 各一处），
    // 故必须在模板判定之前生成回响；失败则空数组，模板照常渲染（分层原则，上层失败不拖垮下层）。
    let echoes: Awaited<ReturnType<typeof generateLinks>>["echoes"] = [];
    try {
      echoes = (await generateLinks(a, title, listRecallSources(assetId))).echoes;
    } catch { /* 没有回响，脉络照常可用 */ }

    const dispatch = await classifyTemplate(a, echoes, transcript, title).catch(() =>
      createArgumentDispatch({
        ...
        takeaways: a.takeaways,
        confidence: a.type_confidence,
        reason: `template-render-exception:${a.video_type}`,
      })
    );
    saveAnalysis({
      assetId,
    ...
    // saveAnalysis 的 REPLACE 会清 echoes 列，故回响紧随其后写回。
    if (echoes.length) {
      saveEchoes(assetId, echoes);
      await addReciprocalEchoes(assetId, echoes); // 反向写回旧视频命中节点，两边互见
    }
...
export type ParseIntake =
  ...
  // 粘的是合集里某一集：反查到它所属的合集，先回给前端让用户选整组还是单条（不建资产）
  | { kind: "confirm-mix"; mixId: string; mixName: string }
  | { kind: "confirm-duplicate"; duplicateCount: number; existingId: string; existingTitle: string };
...
export async function intakeInput(
  input: string,
  opts?: { expandMix?: boolean; forceSingle?: boolean; dedupe?: boolean; overwrite?: boolean },
): Promise<ParseIntake> {
  ...
  if (urls.length > 1) {
    ...
    const normalized = finals.map(normalizeSourceUrl);
    const duplicates = normalized
      .map((sourceUrl) => findLatestAssetBySourceUrl(sourceUrl))
      .filter((asset): asset is SourceAsset => asset !== null);
    if (opts?.dedupe !== false && duplicates.length && !opts?.overwrite) {
      return {
    ...
    const assets = normalized.map((sourceUrl) => {
      const existing = opts?.overwrite ? findLatestAssetBySourceUrl(sourceUrl) : null;
      return existing
        ? resetAssetForOverwrite(existing.id, { sourceUrl, groupId })
        : createAsset({ sourceUrl, groupId });
    });
  ...
  // 合集分享链接常落在单集页（/video/{id}），不带 /mix/ 路径。默认反查这一集所属的合集：
  //   expandMix  → 直接按整组解析（api 侧自动展开用）
  //   都不带     → 命中就回 confirm-mix，让前端问用户「整组还是单条」
  //   forceSingle→ 用户已选「只这条」，跳过反查当单集
  if (!mixId && !opts?.forceSingle) {
    const awemeId = detectAwemeId(finalUrl);
    if (awemeId) {
      const mix = await lookupMixOfVideo(awemeId);
      if (mix) {
        if (opts?.expandMix) { mixId = mix.mixId; mixName = mix.mixName; }
        else return { kind: "confirm-mix", mixId: mix.mixId, mixName: mix.mixName };
      }
    }
  }

  if (mixId) {
    const collectionUrl = `https://www.douyin.com/collection/${mixId}`;
    const existingCollection = findCollectionBySourceUrl(collectionUrl);
    if (opts?.dedupe !== false && existingCollection && !opts?.overwrite) {
      const first = getAssetsByGroup(existingCollection.id)[0];
      return {
        kind: "confirm-duplicate",
        duplicateCount: Math.max(1, getAssetsByGroup(existingCollection.id).length),
        existingId: existingCollection.id,
        existingTitle: existingCollection.name || first?.title || "已解析合集",
      };
    }
    const enumerated = await enumerateMix(mixId);
    let videos = enumerated.videos;
    if (!videos.length) throw new Error("合集里没枚举到视频");
    if (MIX_LIMIT > 0) videos = videos.slice(0, MIX_LIMIT);
    if (!mixName) mixName = enumerated.mixName;
    const groupId = existingCollection && opts?.overwrite
    ...
    setCollectionSourceUrl(groupId, collectionUrl);
    if (enumerated.coverUrl) {
      try {
        const cover = await downloadCover(enumerated.coverUrl, `collection-${groupId}`);
        if (cover) setCollectionCover(groupId, cover);
      } catch { /* 合集仍可用，展示时回退到首集封面 */ }
    }
    const nextSources = videos.map((v) => `https://www.douyin.com/video/${v.aweme_id}`);
    if (existingCollection && opts?.overwrite) deleteGroupAssetsExcept(groupId, nextSources);
    const assets = videos.map((v, index) => {
      const sourceUrl = nextSources[index];
      const existing = opts?.overwrite ? findLatestAssetBySourceUrl(sourceUrl) : null;
      const assetInput = { sourceUrl, title: cleanVideoTitle(v.desc || ""), groupId };
      return existing
        ? resetAssetForOverwrite(existing.id, assetInput)
  ...
  const sourceUrl = normalizeSourceUrl(finalUrl);
  const existing = findLatestAssetBySourceUrl(sourceUrl);
  if (opts?.dedupe !== false && existing && !opts?.overwrite) {
    return {
      kind: "confirm-duplicate",
      duplicateCount: 1,
  ...
  return {
    kind: "single",
    asset: existing && opts?.overwrite
      ? resetAssetForOverwrite(existing.id, { sourceUrl })
