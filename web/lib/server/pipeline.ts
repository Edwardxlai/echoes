...
import {
  ...
  getAnalysis, saveSynthesis, saveCollectionGapFill, saveCollectionExtend,
...
import type { Synthesis, SynthesisPoint, CollectionGapFill, CollectionExtendItem } from "@/lib/data";
...
const EXPAND_SYS = `你是"回响"的认知拓展引擎。基于一条视频的脉络分析结果，生成"补缺"和"延伸"两块内容。只输出 JSON 对象，不要多余文字。

两块内容的分工必须泾渭分明（本任务最重要的约束）：
- 补缺 = 往内夯实：这条视频「承重、却默认你已会」的那块地基。视频把结论压在某个概念/机制/事实上，却没铺——你把它戳破，并用视频之外的公认背景知识补上。只读这一条视频自己。
...
输出：{"gap":"…","fill":"…","searchTerms":["…","…"],"extend":[{"question":"…","hint":"…"}]}
gap、fill 与 extend 均须非空。字符串内部的引号、换行必须转义，务必输出合法 JSON。`;
...
/** 按合集当前成员重跑 L6 合成 + 补缺/延伸（聚类增员后刷新用，人工「移动」也复用）。 */
export async function resynthesizeCollection(collectionId: string): Promise<void> {
  ...
  try { await runCollectionExtend(collectionId, videos, synthesis.seriesQuestion); }
  catch { /* 无合集延伸 */ }
}
...
2. 只补公认、可查证的相邻背景（相近历史、平行案例、不同视角的既有书写），不碰需要选边站的评价——那是延伸的地盘。
...
const COLLECTION_EXTEND_SYS = `你是"回响"的合集延伸引擎。输入是一个合集里所有视频合起来讲了什么（series_question + videos），以及这些视频在各自单条解析里【已经问过】的延伸问题（already_asked）。你要抛出的，是这一整组作为整体、共同引出、却没有一条视频单独够得着的那个更大的开放议题。只输出 JSON 对象，不要多余文字。

延伸 = 往外发散，站在整组的高度（本任务最重要的约束）：
- 单视频延伸顺着"某一条视频自己的逻辑"往下追一步。你不同——你要问的是"把这几条摞在一起、当成一个整体来看，它们共同指向、却谁都没单独触及的那个开放议题"。
- 典型：各讲一段史实，合起来逼出"这段历史该由谁来书写、按谁的标准评价"；几套并行机制摆在一起，逼出"该在哪种价值排序下取舍"。它没有唯一答案，不靠查证解决，等用户带着自己的判断进来讨论。

输出 extend：取 2~3 条，每条 {question, hint}：
- question：**一句话的开放问题**，一个问号收尾，像在黑板上写下一道供全场讨论的题。干净利落，就一个追问——不许堆「还是说…」「如果两者都有，哪个更…」这种多重从句，不写成一大段论述题。立场之间有张力、值得讨论，不判对错。
- hint：2~3 句，点破摆在一起才浮现的那半张缺口、或共同藏着的隐含假设，给追问方向，但不替用户下结论。
- 两条之间必须咬**不同**的隐含假设/立场缺口，不许两条绕着同一个二分（如「人物 vs 结构」）反复问。

延伸条目的标准写法（风格样例，注意 question 都是一句话、且全程不提「视频」二字，照这个register写，别照抄内容）：
{"question":"乱世里，一个人的德行到底能不能挡住时代的塌方？","hint":"几段史事摆在一起会发现：贤者拦不住崩坏，庸人也未必是祸首。那问题就来了——把责任压在个人品性上，是不是本身就选错了尺子？"}
{"question":"「统一」之后紧跟着大乱，这是偶然的坏运气，还是大一统本身就埋着的账？","hint":"皇权、宗室、外戚、士族这几股力量的平衡术反复失手，暗示的也许不是某一朝无能，而是这套结构在承平时就欠下的债，迟早要还。"}

护栏（必须遵守）：
1. **禁止自指（最容易犯，务必守住）**：question 与 hint 里绝不许出现「视频／这些视频／视频中／这组／那组／整组／这几条」这类字眼，一个都不行——读者看得见自己在看什么，你只把议题本身当作一道公开的题写出来。要点破共同缺口时，直接陈述那件事，不要说「这些视频都没讲」。
2. 不得重复 already_asked 里【单视频已经问过】的问题——那些已在各自解析页出现，这里再问就是冗余。你要的是只有并置起来才浮现的、更高一层的议题。
3. 是开放议题、不是可查证的背景（那是补缺的地盘），也不替用户拍板下结论。

门控：若合起来实在没有引出值得并置讨论的更高层议题 → extend 输出空数组 []。宁缺毋滥。

行文用中文标点，引用与强调用「」，不用英文引号或弯引号。输出：{"extend":[{"question":"…","hint":"…"}]}
字符串内部的引号、换行必须转义，务必输出合法 JSON。`;

/** 合集级延伸：整组之上的开放议题，排除各视频单条已问过的。门控/失败时返回空数组。 */
export async function generateCollectionExtend(
  videos: SynthesisInputVideo[],
  seriesQuestion: string,
  alreadyAsked: string[],
): Promise<CollectionExtendItem[]> {
  const input = JSON.stringify({
    series_question: seriesQuestion,
    videos: videos.map((v) => ({
      title: v.title,
  ...
  const attempt = async (): Promise<CollectionExtendItem[]> => {
    const parsed = await callDeepseekJson<{ extend?: unknown }>(COLLECTION_EXTEND_SYS, input, 0.4);
    return (Array.isArray(parsed.extend) ? parsed.extend : [])
      .map((x) => ({
        question: String((x as CollectionExtendItem)?.question ?? "").trim(),
        hint: String((x as CollectionExtendItem)?.hint ?? "").trim(),
      }))
      .filter((x) => x.question && x.hint)
      .slice(0, 3);
  };
...
/** L6 收尾：基于整组 + 各视频单条已问过的延伸，生成合集延伸并落库。门控空则不存。 */
async function runCollectionExtend(
  collectionId: string,
  videos: SynthesisInputVideo[],
  seriesQuestion: string,
): Promise<void> {
  const alreadyAsked = videos
    .flatMap((v) => getAnalysis(v.id)?.cognitiveExpansion?.extend ?? [])
    .map((e) => e.question)
    .filter(Boolean);
  const extend = await generateCollectionExtend(videos, seriesQuestion, alreadyAsked);
  if (extend.length) saveCollectionExtend(collectionId, extend);
}

export { runCollectionGapFill, runCollectionExtend };
...
export async function runAssetPipeline(assetId: string): Promise<void> {
  ...
  try {
    ...
    // L4：补缺+延伸（补缺读本条视频、延伸往外发散，都不牵扯观看史——历史连接归回响）
    try {
...
async function finalizeMixGroup(groupId: string): Promise<void> {
  ...
  try {
    ...
    if (videos.length >= 2) {
      ...
      // 合集补缺/延伸各自独立分层：失败只是缺席对应那半，不连累已存的合成
      try { await runCollectionGapFill(groupId, videos, synthesis.seriesQuestion); }
      catch { /* 无合集补缺 */ }
      try { await runCollectionExtend(groupId, videos, synthesis.seriesQuestion); }
      catch { /* 无合集延伸 */ }
    }
